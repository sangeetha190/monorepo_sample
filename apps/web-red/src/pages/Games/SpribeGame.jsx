...
const SpribeGame = () => {
    ...
    const filteredGames = games.filter(
        (game) => game.name && game.name.toLowerCase().includes((searchTerm || "").toLowerCase())
    );
    // Fetch All Games
    ...
    const handleGameClick = async (game) => {
        ...
        try {
            ...
            const response = await axios.get(`${BASE_URL}/player/spribe/${game.name}`, {
                                    ...
                                    >
                                        <div className="game-card-wrapper rounded-2 new-cardclr mt-2">
                                            <div className="game-card p-0 m-0 p-1">
                                                <img src={game.image} className="game-card-img" alt={game.image} />
                                                <div className="d-flex flex-column text-white text-center py-2 px-1">
                                                    <span className="fs-12 fw-bold text-truncate">{game.name}</span>
                                                </div>
                                            </div>
