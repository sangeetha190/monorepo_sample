...
export const AuthProvider = ({children}) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const msg = err?.response?.data?.error || err?.message || "Gift link invalid, used, or claim failed.";
                sessionStorage.setItem("giftFlash", JSON.stringify({type: "error", message: msg}));
    ...
    const fetchPortalStatus = async () => {
        try {
            ...
            const map = Object.fromEntries((res?.data?.channels || []).map((c) => [c.channel, Number(!!c.status)]));
    ...
    const fetchCommunication = async () => {
        try {
            ...
            // 1) on/off
            const map = Object.fromEntries((data?.channels || []).map((c) => [c.channel, Number(!!c.status)]));
            setPortalChannels({whatsapp: 0, tawk: 0, telegram: 0, ...map});
